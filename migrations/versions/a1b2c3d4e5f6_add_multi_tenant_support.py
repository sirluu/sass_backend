...
def upgrade():
    ...
    op.execute(
        sa.text(
            """
            INSERT INTO tenants (id, name, slug, domain, description, settings, is_active, created_at, updated_at)
            VALUES (
                :id,
                'Sơn Phong Thủy',
                'son-phong-thuy',
    ...
    op.execute(
        sa.text("UPDATE products SET tenant_id = :tenant_id WHERE tenant_id IS NULL").bindparams(
            tenant_id=DEFAULT_TENANT_ID
        )
    )
    ...
    op.execute(
        sa.text("UPDATE cart SET tenant_id = :tenant_id WHERE tenant_id IS NULL").bindparams(
            tenant_id=DEFAULT_TENANT_ID
        )
    )
    ...
    op.execute(
        sa.text(
            "UPDATE chat_sessions SET tenant_id = :tenant_id WHERE tenant_id IS NULL"
    ...
    op.execute(
        sa.text(
            "UPDATE user_likes SET tenant_id = :tenant_id WHERE tenant_id IS NULL"
        ).bindparams(tenant_id=DEFAULT_TENANT_ID)
    )
